...
namespace Loretta.Generators.SyntaxFactsGenerator
{
    ...
    {
        // @formatter:off
        private const string SyntaxKindAttributes = 
            """
        ...
        public static readonly SourceText SyntaxKindAttributesText = SourceText.From(
            SyntaxKindAttributes,
            Encoding.UTF8);

        public static KindList? ExtractKindList(SourceProductionContext context, WantedSymbols symbols)
        {
            if (symbols.SyntaxKindType is null) return null;

            var fields = symbols.SyntaxKindType.GetMembers().OfType<IFieldSymbol>().ToImmutableArray();
            ...
            ImmutableArray<IFieldSymbol> fields)
        {
            ...
            foreach (var field in fields)
            {
                ...
                var tokenInfo          = GetTokenInfo(symbols.TokenAttributeType, symbols.KeywordAttributeType, field);
                var unaryOperatorInfo  = GetOperatorInfo(symbols.UnaryOperatorAttributeType, field);
                var binaryOperatorInfo = GetOperatorInfo(symbols.BinaryOperatorAttributeType, field);
                var extraCategories    = GetExtraCategories(symbols.ExtraCategoriesAttributeType, field, context);
                var properties         = GetProperties(symbols.PropertyAttributeType, field, context);
                ...
                if ((unaryOperatorInfo is not null || binaryOperatorInfo is not null)
                    && string.IsNullOrWhiteSpace(tokenInfo?.Text))
                {
                    hasErrors = true;
        ...
        private static TokenInfo? GetTokenInfo(
            ...
            IFieldSymbol     field)
        {
            if (Utilities.GetAttribute(field, keywordAttributeType) is { } keywordAttributeData)
            {
                var text = keywordAttributeData.ConstructorArguments.Single().Value as string;
                if (string.IsNullOrWhiteSpace(text)) text = null;
                return new TokenInfo(text, isKeyword: true);
            }

            if (Utilities.GetAttribute(field, tokenAttributeType) is not { } tokenAttributeData) return null;
            var text2 =
                tokenAttributeData.NamedArguments.SingleOrDefault(static kv => kv.Key == "Text").Value.Value as string;
            if (string.IsNullOrWhiteSpace(text2)) text2 = null;
            return new TokenInfo(text2, isKeyword: false);
        }

        private static OperatorInfo? GetOperatorInfo(INamedTypeSymbol operatorAttributeType, IFieldSymbol field)
        {
            ...
            SourceProductionContext context)
        {
            ...
            if (attr is null) return ImmutableArray<string>.Empty;

            if (attr.ApplicationSyntaxReference is not null)
            {
                var attrSyntax =
                    (AttributeSyntax) attr.ApplicationSyntaxReference!.GetSyntax(context.CancellationToken);
                if (attrSyntax.ArgumentList is not null)
                {
                    foreach (var arg in attrSyntax.ArgumentList!.Arguments)
                    {
                        if (arg.Expression is not MemberAccessExpressionSyntax { Expression: SimpleNameSyntax baseName }
                            || !baseName.Identifier.Text.Equals("SyntaxKindCategory", StringComparison.Ordinal))
                        {
                            context.ReportDiagnostic(
                                Diagnostic.Create(Diagnostics.CategoryNotInConstantClass, location: arg.GetLocation()));
                        }
                    }
                }
            }

            var categories = attr.ConstructorArguments.Single().Values.Select(static arg => (string) arg.Value!)
                                 .ToImmutableArray();
            return categories;
        }

        private static ImmutableDictionary<string, TypedConstant> GetProperties(
            INamedTypeSymbol        propertyAttributeType,
            IFieldSymbol            field,
            SourceProductionContext context)
        {
            var attributes = Utilities.GetAttributes(field, propertyAttributeType);
            if (attributes.IsEmpty) return ImmutableDictionary<string, TypedConstant>.Empty;

            foreach (var attrSyntax in attributes.Select(
                         attribute
                             => attribute.ApplicationSyntaxReference?.GetSyntax(context.CancellationToken) as
                                    AttributeSyntax))
            {
                if (attrSyntax?.ArgumentList is null) continue;
                ...
                {
                    context.ReportDiagnostic(
                        Diagnostic.Create(Diagnostics.PropertyNotInConstantClass, location: arg.GetLocation()));
                }
            }

            var properties = attributes.Select(
                static attr => new KeyValuePair<string, TypedConstant>(
                    key: (string) attr.ConstructorArguments[0].Value!,
                    value: attr.ConstructorArguments[1]));
