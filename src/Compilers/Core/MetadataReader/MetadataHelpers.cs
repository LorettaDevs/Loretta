...
namespace Loretta.CodeAnalysis
{
    internal static class MetadataHelpers
    {
        ...
        private struct SerializedTypeDecoder
        {
            ...
            {
                RoslynDebug.Assert(!isTypeArgumentWithAssemblyName || isTypeArgument);
                ...
                while (!EndOfInput)
                {
                    ...
                    if (i >= 0)
                    {
                        ...
                        RoslynDebug.Assert(decodedString != null);
...
ExitDecodeTypeName:
            ...
            private static void HandleDecodedTypeName(string decodedTypeName, bool decodingTopLevelType, ref string topLevelType, ref ArrayBuilder<string> nestedTypesBuilder)
            {
                if (decodedTypeName.Length != 0)
                {
                    ...
                    {
                        RoslynDebug.Assert(topLevelType == null);
            ...
            {
                RoslynDebug.Assert(i == _input.Length || s_typeNameDelimiters.Contains(_input[i]));
        ...
        {
            RoslynDebug.Assert(emittedTypeName != null, "NULL actual name unexpected!!!");
        ...
        internal static string InferTypeArityAndUnmangleMetadataName(string emittedTypeName, out short arity)
        {
            ...
            if (arity == 0)
            {
                RoslynDebug.Assert(suffixStartsAt == -1);
                return emittedTypeName;
            }

            RoslynDebug.Assert(suffixStartsAt > 0 && suffixStartsAt < emittedTypeName.Length - 1);
        ...
        {
            RoslynDebug.Assert(pstrName != null);
            ...
            RoslynDebug.Assert(angleBracketDepth == 0);
            ...
            out IEnumerable<KeyValuePair<string, IEnumerable<IGrouping<string, TypeDefinitionHandle>>>> namespaces)
        {
            ...
            RoslynDebug.Assert(namespaceNameLength >= 0);
            RoslynDebug.Assert(!isGlobalNamespace || namespaceNameLength == 0);
                    ...
                    do
                    {
                        ...
                        else
                        {
                            ...
                            if (cmp > 0)
                            {
                                ...
                                RoslynDebug.Assert((object)lastChildNamespaceName != null);
                            ...
                            {
                                RoslynDebug.Assert(typesInLastChildNamespace.Count != 0);
...
/*empty statement*/
            ...
            if (possiblyHavePairsWithDuplicateKey)
            {
                ...
                if (names.Count != nestedNamespaces.Count) // nothing to merge otherwise
                {
                    ...
                    RoslynDebug.Assert(removed > 0);
                }
            }

            namespaces = nestedNamespaces;

            RoslynDebug.Assert(types != null);
            RoslynDebug.Assert(namespaces != null);
        }
