...
namespace Loretta.CodeAnalysis
{
    ...
    internal sealed class PEModule : IDisposable
    {
        ...
        internal PEModule(ModuleMetadata owner, PEReader peReader, IntPtr metadataOpt, int metadataSizeOpt, bool includeEmbeddedInteropTypes, bool ignoreAssemblyRefs)
        {
            ...
            RoslynDebug.Assert((peReader == null) ^ (metadataOpt == IntPtr.Zero && metadataSizeOpt == 0));
            RoslynDebug.Assert(metadataOpt == IntPtr.Zero || metadataSizeOpt > 0);
        ...
        private sealed class PEHashProvider : CryptographicHashProvider
        {
            ...
            {
                RoslynDebug.Assert(peReader != null);
                _peReader = peReader;
            }
        ...
        private unsafe void InitializeMetadataReader()
        {
            ...
            {
                RoslynDebug.Assert(_peReaderOpt != null);
        ...
        {
            RoslynDebug.Assert(_hashesOpt != null);
        ...
        public string GetTypeDefNameOrThrow(TypeDefinitionHandle typeDef)
        {
            ...
            RoslynDebug.Assert(name.Length == 0 || MetadataHelpers.IsValidMetadataIdentifier(name)); // Obfuscated assemblies can have types with empty names.
            ...
            public int Compare(IGrouping<string, TypeDefinitionHandle> left, IGrouping<string, TypeDefinitionHandle> right)
            {
                ...
                if (result == 0)
                {
                    ...
                    if (result == 0)
                    {
                        ...
                        RoslynDebug.Assert(left.IsEmpty() && right.IsEmpty());
        ...
        internal bool HasDynamicAttribute(EntityHandle token, out ImmutableArray<bool> transformFlags)
        {
            ...
            RoslynDebug.Assert(!info.HasValue || info.SignatureIndex == 0 || info.SignatureIndex == 1);
...
#nullable disable
        ...
        {
            RoslynDebug.Assert(description.Namespace == "System.Diagnostics.CodeAnalysis");
            RoslynDebug.Assert(description.Name == "MaybeNullWhenAttribute" || description.Name == "NotNullWhenAttribute" || description.Name == "DoesNotReturnIfAttribute");
        ...
        internal CustomAttributeHandle GetAttributeUsageAttributeHandle(EntityHandle token)
        {
            ...
            RoslynDebug.Assert(info.SignatureIndex == 0);
            return info.Handle;
        }
...
#nullable enable
        ...
        {
            RoslynDebug.Assert(attributeInfo.HasValue);
        ...
        {
            RoslynDebug.Assert(!handle.IsNil);
            try
            {
...
#nullable disable
        ...
        {
            RoslynDebug.Assert(info.HasValue);
...
#nullable disable
        ...
        internal struct AttributeInfo
        {
            ...
            {
                RoslynDebug.Assert(signatureIndex >= 0 && signatureIndex <= byte.MaxValue);
                this.Handle = handle;
        ...
        private static bool IsTargetAttribute(
            ...
            bool ignoreCase)
        {
            RoslynDebug.Assert(namespaceName != null);
        ...
        {
            RoslynDebug.Assert(assemblyName != null);
        ...
        internal EntityHandle GetTypeRef(
            ...
            string typeName)
        {
            RoslynDebug.Assert(!resolutionScope.IsNil);
            RoslynDebug.Assert(namespaceName != null);
            RoslynDebug.Assert(typeName != null);
            ...
            out EntityHandle resolutionScope)
        {
            ...
            RoslynDebug.Assert(MetadataHelpers.IsValidMetadataIdentifier(name));
        ...
        private static int GetTargetAttributeSignatureIndex(MetadataReader metadataReader, CustomAttributeHandle customAttribute, AttributeDescription description)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < description.Signatures.Length; i++)
                {
                    ...
                    RoslynDebug.Assert(targetSignature.Length >= 3);
                    sig.Reset();
            ...
            out EntityHandle attributeCtor)
        {
            try
            {
                ...
                else if (attributeCtor.Kind == HandleKind.MethodDefinition)
                {
                    ...
                    RoslynDebug.Assert(!ctorType.IsNil);
                }
                else
                {
                    // invalid metadata
        ...
        {
            RoslynDebug.Assert(_lazyContainsNoPiaLocalTypes != ThreeState.True);
        ...
        internal bool HasNullableContextAttribute(EntityHandle token, out byte value)
        {
            ...
            RoslynDebug.Assert(!info.HasValue || info.SignatureIndex == 0);
        ...
        {
            RoslynDebug.Assert(!param.IsNil);
        ...
        {
            RoslynDebug.Assert(!fieldDef.IsNil);
            ...
            if (_lazyForwardedTypesToAssemblyIndexMap == null)
            {
                ...
                try
                {
                    ...
                    foreach (var handle in forwarders)
                    {
                        ...
                        {
                            RoslynDebug.Assert(indices.FirstIndex >= 0, "Not allowed to store a negative (non-existent) index in typesToAssemblyIndexMap");
