...
namespace Loretta.CodeAnalysis
{
    ...
    internal abstract partial class Operation : IOperation
    {
        ...
        protected Operation(SemanticModel? semanticModel, SyntaxNode syntax, bool isImplicit)
        {
            ...
            {
                RoslynDebug.Assert(semanticModel.ContainingModelOrSelf != null);
                if (semanticModel.IsSpeculativeSemanticModel)
                {
                    RoslynDebug.Assert(semanticModel.ContainingModelOrSelf == semanticModel);
                }
                else
                {
                    RoslynDebug.Assert(semanticModel.ContainingModelOrSelf != semanticModel);
                    RoslynDebug.Assert(semanticModel.ContainingModelOrSelf.ContainingModelOrSelf == semanticModel.ContainingModelOrSelf);
                }
            }
#endif
        ...
        public IOperation? Parent
        {
            get
            {
                RoslynDebug.Assert(_parentDoNotAccessDirectly != s_unset, "Attempt to access parent node before construction is complete!");
        ...
        {
            RoslynDebug.Assert(_parentDoNotAccessDirectly == s_unset);
            RoslynDebug.Assert(parent != s_unset);
            _parentDoNotAccessDirectly = parent;

            // tree must belong to same semantic model if parent is given
            RoslynDebug.Assert(parent == null || ((Operation)parent).OwningSemanticModel == OwningSemanticModel);
        }
        ...
        internal static void VerifyParentOperation(IOperation? parent, IOperation child)
        {
            if (child is object)
            {
                RoslynDebug.Assert((object?)child.Parent == parent);
            }
        }
        ...
        {
            RoslynDebug.Assert(!children.IsDefault);
