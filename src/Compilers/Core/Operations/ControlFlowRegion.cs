...
namespace Loretta.CodeAnalysis.FlowAnalysis
{
    ...
    public sealed class ControlFlowRegion
    {
        ...
        {
            RoslynDebug.Assert(firstBlockOrdinal >= 0);
            RoslynDebug.Assert(lastBlockOrdinal >= firstBlockOrdinal);
            ...
            {
                RoslynDebug.Assert(r.EnclosingRegion == null && r.Kind != ControlFlowRegionKind.Root);
            ...
            switch (kind)
            {
                ...
                case ControlFlowRegionKind.FilterAndHandler:
                    RoslynDebug.Assert(NestedRegions.Length == 2);
                    RoslynDebug.Assert(NestedRegions[0].Kind == (kind == ControlFlowRegionKind.TryAndFinally ? ControlFlowRegionKind.Try : ControlFlowRegionKind.Filter));
                    RoslynDebug.Assert(NestedRegions[1].Kind == (kind == ControlFlowRegionKind.TryAndFinally ? ControlFlowRegionKind.Finally : ControlFlowRegionKind.Catch));
                    RoslynDebug.Assert(NestedRegions[0].FirstBlockOrdinal == firstBlockOrdinal);
                    RoslynDebug.Assert(NestedRegions[1].LastBlockOrdinal == lastBlockOrdinal);
                    RoslynDebug.Assert(NestedRegions[0].LastBlockOrdinal + 1 == NestedRegions[1].FirstBlockOrdinal);
                    break;

                case ControlFlowRegionKind.TryAndCatch:
                    RoslynDebug.Assert(NestedRegions.Length >= 2);
                    RoslynDebug.Assert(NestedRegions[0].Kind == ControlFlowRegionKind.Try);
                    RoslynDebug.Assert(NestedRegions[0].FirstBlockOrdinal == firstBlockOrdinal);
                    ...
                    for (int i = 1; i < NestedRegions.Length; i++)
                    {
                        ...
                        RoslynDebug.Assert(previousLast + 1 == r.FirstBlockOrdinal);
                        previousLast = r.LastBlockOrdinal;

                        RoslynDebug.Assert(r.Kind == ControlFlowRegionKind.FilterAndHandler || r.Kind == ControlFlowRegionKind.Catch);
                    }

                    RoslynDebug.Assert(previousLast == lastBlockOrdinal);
                    break;
                ...
                case ControlFlowRegionKind.ErroneousBody:
                    ...
                    foreach (ControlFlowRegion r in NestedRegions)
                    {
                        RoslynDebug.Assert(previousLast < r.FirstBlockOrdinal);
                        previousLast = r.LastBlockOrdinal;
                    }
