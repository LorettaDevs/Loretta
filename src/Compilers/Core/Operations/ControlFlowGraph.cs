...
namespace Loretta.CodeAnalysis.FlowAnalysis
{
    ...
    public sealed partial class ControlFlowGraph
    {
        ...
        {
            RoslynDebug.Assert(parent != null == (originalOperation.Kind == OperationKind.LocalFunction || originalOperation.Kind == OperationKind.AnonymousFunction));
            RoslynDebug.Assert(captureIdDispenser != null);
            RoslynDebug.Assert(!blocks.IsDefault);
            RoslynDebug.Assert(blocks.First().Kind == BasicBlockKind.Entry);
            RoslynDebug.Assert(blocks.Last().Kind == BasicBlockKind.Exit);
            RoslynDebug.Assert(root != null);
            RoslynDebug.Assert(root.Kind == ControlFlowRegionKind.Root);
            RoslynDebug.Assert(root.FirstBlockOrdinal == 0);
            RoslynDebug.Assert(root.LastBlockOrdinal == blocks.Length - 1);
            RoslynDebug.Assert(!localFunctions.IsDefault);
            RoslynDebug.Assert(localFunctionsMap != null);
            RoslynDebug.Assert(localFunctionsMap.Count == localFunctions.Length);
            RoslynDebug.Assert(localFunctions.Distinct().Length == localFunctions.Length);
            RoslynDebug.Assert(anonymousFunctionsMap != null);
#if DEBUG
            foreach (IMethodSymbol method in localFunctions)
            {
                RoslynDebug.Assert(method.MethodKind == MethodKind.LocalFunction);
                RoslynDebug.Assert(localFunctionsMap.ContainsKey(method));
            }
#endif 
...
#pragma warning restore RS0026 // Do not add multiple public overloads with optional parameters
        ...
        internal static ControlFlowGraph CreateCore(IOperation operation, string argumentNameForException, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                RoslynDebug.Assert(controlFlowGraph.OriginalOperation == operation);
        ...
        internal bool TryGetLocalFunctionControlFlowGraph(IMethodSymbol localFunction, CancellationToken cancellationToken, [NotNullWhen(true)] out ControlFlowGraph? controlFlowGraph)
        {
            ...
            RoslynDebug.Assert(localFunction == LocalFunctions[info.ordinal]);
        ...
        internal bool TryGetAnonymousFunctionControlFlowGraph(IFlowAnonymousFunctionOperation anonymousFunction, CancellationToken cancellationToken, [NotNullWhen(true)] out ControlFlowGraph? controlFlowGraph)
        {
            ...
            if (anonymousFlowGraph == null)
            {
                ...
                RoslynDebug.Assert(graph.OriginalOperation == anonymous.Original);
