        ...
        private class PerAnalyzerState
        {
            ...
            public bool HasPendingSyntaxAnalysis(SourceOrAdditionalFile? file)
            {
                lock (_gate)
                {
                    ...
                    RoslynDebug.Assert(_lazyFilesWithAnalysisData != null);
                ...
                where TAnalysisEntity : notnull
            {
                    ...
                    (state == null || state.StateKind == StateKind.ReadyToProcess))
                {
                    ...
                    RoslynDebug.Assert(state.StateKind == StateKind.InProcess);
            ...
            {
                RoslynDebug.Assert(_pendingDeclarations[symbol] == declarationDataMap);
            ...
            {
                RoslynDebug.Assert(declarationsCount > 0);
                if (!_pendingDeclarations.TryGetValue(symbol, out var declarationDataMap))
                {
                    return true;
                }
            ...
            {
                RoslynDebug.Assert(_lazyPendingSymbolEndAnalyses != null);
