        ...
        private sealed class AnalyzerExecutionContext
        {
                ...
                out (ImmutableArray<SymbolEndAnalyzerAction> symbolEndActions, SymbolDeclaredCompilationEvent symbolDeclaredEvent) containerEndActionsAndEvent)
            {
                ...
                lock (_gate)
                {
                    ...
                    RoslynDebug.Assert(pendingMemberSymbols != null);
            ...
            {
                RoslynDebug.Assert(!symbolEndActions.IsEmpty);

                var symbol = symbolDeclaredEvent.Symbol;
                lock (_gate)
                {
                    RoslynDebug.Assert(_lazyPendingMemberSymbolsMap != null);
            ...
            public void VerifyAllSymbolEndActionsExecuted()
            {
                lock (_gate)
                {
                    RoslynDebug.Assert(_lazyPendingMemberSymbolsMap == null || _lazyPendingMemberSymbolsMap.Count == 0);
                    RoslynDebug.Assert(_lazyPendingSymbolEndActionsMap == null || _lazyPendingSymbolEndActionsMap.Count == 0);
                }
            }
        }
    }
