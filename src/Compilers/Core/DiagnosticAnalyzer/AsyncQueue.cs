...
retry:
            ...
            lock (SyncObject)
            {
                ...
                RoslynDebug.Assert(_data.Count == 0);
                waiter = _waiters.Dequeue();
            }

            RoslynDebug.Assert(waiter.Task.CreationOptions.HasFlag(TaskCreationOptions.RunContinuationsAsynchronously));
        ...
        private bool CompleteCore()
        {
            ...
            if (existingWaiters?.Count > 0)
            {
                ...
                RoslynDebug.Assert(this.Count == 0, "we should not be cancelling the waiters when we have items in the queue");
                foreach (var tcs in existingWaiters)
                {
                    RoslynDebug.Assert(tcs.Task.CreationOptions.HasFlag(TaskCreationOptions.RunContinuationsAsynchronously));
                    tcs.TrySetResult(default);
                }
            }

            RoslynDebug.Assert(_whenCompleted.Task.CreationOptions.HasFlag(TaskCreationOptions.RunContinuationsAsynchronously));
        ...
        private static void AttachCancellation<T>(TaskCompletionSource<T> taskCompletionSource, CancellationToken cancellationToken)
        {
            ...
            RoslynDebug.Assert(taskCompletionSource.Task.CreationOptions.HasFlag(TaskCreationOptions.RunContinuationsAsynchronously));
