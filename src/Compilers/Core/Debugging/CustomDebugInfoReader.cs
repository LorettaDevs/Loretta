...
namespace Loretta.CodeAnalysis.Debugging
{
    ...
    internal static class CustomDebugInfoReader
    {
        ...
        public static ImmutableArray<DynamicLocalInfo> DecodeDynamicLocalsRecord(ImmutableArray<byte> bytes)
        {
            ...
            for (var i = 0; i < bucketCount; i++)
            {
                RoslynDebug.Assert(flagsBuilder.Count == 0);
                RoslynDebug.Assert(nameBuilder.Length == 0);
...
RETRY:
            ...
            foreach (var record in GetCustomDebugInfoRecords(bytes))
            {
                switch (record.Kind)
                {
                    ...
                    case CustomDebugInfoKind.ForwardModuleInfo:
                        ...
                        RoslynDebug.Assert(!allModuleInfoImportStrings.IsDefault);
            ...
            RoslynDebug.Assert(!importStrings.IsDefault);
            ...
            if (externAliasStrings.IsDefault)
            {
                RoslynDebug.Assert(groupBuilder.Count == 0);
