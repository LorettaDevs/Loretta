...
namespace Loretta.Utilities
{
        ...
        where T : class
    {
        ...
        private void Resize()
        {
            RoslynDebug.Assert(_size == _items.Length);
            RoslynDebug.Assert(_items.Length == 0 || _items.Length >= MinimalNonEmptySize);
            ...
            RoslynDebug.Assert(_items.Length > 0 && _size < 3 * _items.Length / 4, "length: " + _items.Length + " size: " + _size);
        }
        ...
        {
            RoslynDebug.Assert(_items[firstDead].IsNull());
