        ...
        where TAssemblySymbol : class, IAssemblySymbolInternal
    {
        ...
        protected struct ResolvedReference
        {
                ...
                : this(index, kind)
            {
                ...
                RoslynDebug.Assert(!aliasesOpt.IsDefault || !recursiveAliasesOpt.IsDefault);
                RoslynDebug.Assert(!mergedReferences.IsDefault);
            ...
            public ImmutableArray<string> AliasesOpt
            {
                get
                {
                    RoslynDebug.Assert(!IsUninitialized);
                    return _aliasesOpt;
                }
            }
            ...
            DiagnosticBag diagnostics)
        {
            ...
            for (int referenceIndex = referenceCount - 1; referenceIndex >= 0; referenceIndex--)
            {
                ...
                RoslynDebug.Assert(metadata != null || diagnostics.HasAnyErrors());
        ...
        private bool TryGetObservedMetadata(PortableExecutableReference peReference, DiagnosticBag diagnostics, out Metadata? metadata)
        {
            ...
            {
                RoslynDebug.Assert(existing is Metadata || existing is Diagnostic);
            ...
            bool supersedeLowerVersions)
        {
            ...
            if (supersedeLowerVersions)
            {
                ...
                {
                    RoslynDebug.Assert(other.Identity is object);
            ...
            if (identity.IsStrongName)
            {
                RoslynDebug.Assert(equivalent.Identity.IsStrongName);
            ...
            RoslynDebug.Assert(equivalent.Reference != null);
            ...
            out ImmutableArray<Location> referenceDirectiveLocations)
        {
            ...
            try
            {
                ...
                {
                    RoslynDebug.Assert(referenceDirective.Location is object);
                    ...
                    RoslynDebug.Assert(referenceDirective.File is object);
                    RoslynDebug.Assert(referenceDirective.Location.SourceTree is object);
        ...
        private static PortableExecutableReference? ResolveReferenceDirective(string reference, Location location, TCompilation compilation)
        {
            ...
            // checked earlier:
            RoslynDebug.Assert(compilation.Options.MetadataReferenceResolver != null);
            ...
            AssemblyIdentityComparer assemblyIdentityComparer)
        {
            ...
            for (int i = definitionStartIndex; i < definitions.Length; i++)
            {
                ...
                switch (assemblyIdentityComparer.Compare(reference, definition))
                {
                    ...
                    case AssemblyIdentityComparer.ComparisonResult.EquivalentIgnoringVersion:
                        ...
                        {
                            RoslynDebug.Assert(reference.Version > definition.Version);
            ...
            {
                RoslynDebug.Assert(definitions[0].Identity.PublicKeyToken.IsEmpty);
