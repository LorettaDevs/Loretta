...
namespace Loretta.CodeAnalysis
{
    internal partial class CommonReferenceManager<TCompilation, TAssemblySymbol>
    {
        ...
        {
            RoslynDebug.Assert(explicitAssemblies[0] is AssemblyDataForAssemblyBeingBuilt);
            RoslynDebug.Assert(explicitReferences.Length == explicitReferenceMap.Length);
            ...
            try
            {
                ...
                RoslynDebug.Assert(referenceBindings.Count == allAssemblies.Length);
        ...
        {
            RoslynDebug.Assert(explicitAssemblies[0] is AssemblyDataForAssemblyBeingBuilt);
            RoslynDebug.Assert(referenceBindings.Count == explicitAssemblies.Length);
            RoslynDebug.Assert(explicitReferences.Length == explicitReferenceMap.Length);
            ...
            try
            {
                while (referenceBindingsToProcess.Count > 0)
                {
                    ...
                    foreach (var binding in bindings)
                    {
                        ...
                        RoslynDebug.Assert(binding.ReferenceIdentity is object);
                ...
                {
                    RoslynDebug.Assert(lazyAliasMap == null);
        ...
        {
            RoslynDebug.Assert(result.Count == 0);
            ...
            RoslynDebug.Assert(bindingIndex == bindingsOfAssemblyBeingBuilt.Length);
            ...
            RoslynDebug.Assert(result.Count == explicitModules.Length + totalReferencedAssemblyCount);
        }
            ...
            [NotNullWhen(true)] out PortableExecutableReference? resolvedReference)
        {
            ...
            RoslynDebug.Assert(resolvedAssembly is object);
            ...
            MetadataImportOptions importOptions)
        {
            ...
            RoslynDebug.Assert(assembly is object);
            ...
            for (int i = 1; i < totalAssemblies; i++)
            {
                ...
                foreach (TAssemblySymbol candidateAssembly in assemblies[i].AvailableSymbols)
                {
                        ...
                        else
                        {
                            ...
                            if (corLibraryIndex < 0)
                            {
                                ...
                                if (GetCorLibrary(candidateInputAssemblySymbols[j]) != null)
                                {
                                    ...
                                    RoslynDebug.Assert(GetCorLibrary(candidateInputAssemblySymbols[j]) == null);
                                    match = false;
                                    break;
                                }
                            }
                            else
                            {
                                ...
                                RoslynDebug.Assert(corLibraryIndex != 0);
                                ...
                                {
                                    // I am leaving the Assert here because it will likely indicate a bug somewhere.
                                    RoslynDebug.Assert(candidateInputAssemblySymbols[corLibraryIndex] == null);
                                    match = false;
                                    break;
                                }
                            }
                    ...
                    if (match)
                    {
                        ...
                        for (int j = 1; j < totalAssemblies; j++)
                        {
                            RoslynDebug.Assert(candidateInputAssemblySymbols[j] != null);
            ...
            for (int i = 1; i < totalAssemblies; i++)
            {
                ...
                foreach (TAssemblySymbol candidateAssembly in assemblies[i].AvailableSymbols)
                {
                    ...
                    while (match && candidatesToExamine.Count > 0)
                    {
                        ...
                        RoslynDebug.Assert(candidate.DefinitionIndex >= 0);
                        RoslynDebug.Assert(candidate.AssemblySymbol is object);

                        int candidateIndex = candidate.DefinitionIndex;

                        // Have we already chosen symbols for the corresponding assembly?
                        RoslynDebug.Assert(boundInputs[candidateIndex].AssemblySymbol == null ||
                        ...
                        RoslynDebug.Assert(candidateInputAssemblySymbols[candidateIndex] == null);
                        ...
                        RoslynDebug.Assert(candidateReferenceBinding is object);
                        RoslynDebug.Assert(candidateReferenceBinding.Length == candidateReferencedSymbols.Count);
                        ...
                        if (match)
                        {
                            ...
                            else
                            {
                                ...
                                RoslynDebug.Assert(ReferenceEquals(candidateCorLibrary, GetCorLibrary(candidateCorLibrary)));
                                ...
                                RoslynDebug.Assert(!assemblies[corLibraryIndex].ContainsNoPiaLocalTypes);
                                RoslynDebug.Assert(!assemblies[corLibraryIndex].IsLinked);
                                RoslynDebug.Assert(!IsLinked(candidateCorLibrary));
                    ...
                    if (match)
                    {
                        ...
                        for (int k = 0; k < totalAssemblies; k++)
                        {
                            ...
                            {
                                RoslynDebug.Assert(boundInputs[k].AssemblySymbol == null);
        ...
        private static bool IsSuperseded(AssemblyIdentity identity, IReadOnlyDictionary<string, List<ReferencedAssemblyIdentity>> assemblyReferencesBySimpleName)
        {
            ...
            RoslynDebug.Assert(value.Identity is object);
        ...
        private static int IndexOfCorLibrary(ImmutableArray<AssemblyData> assemblies, IReadOnlyDictionary<string, List<ReferencedAssemblyIdentity>> assemblyReferencesBySimpleName, bool supersedeLowerVersions)
        {
            ...
            for (int i = 1; i < assemblies.Length; i++)
            {
                ...
                // Loretta.CodeAnalysis.VisualBasic.CommandLineCompiler.ResolveMetadataReferencesFromArguments
