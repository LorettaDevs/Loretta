...
namespace Loretta.CodeAnalysis
{
    ...
    internal partial class CommonReferenceManager<TCompilation, TAssemblySymbol> : CommonReferenceManager
    {
        ...
        {
            RoslynDebug.Assert(simpleAssemblyName != null);
            RoslynDebug.Assert(identityComparer != null);
        ...
        internal ImmutableDictionary<MetadataReference, ImmutableArray<MetadataReference>> MergedAssemblyReferencesMap
        {
            ...
            {
                AssertBound();
                RoslynDebug.Assert(_lazyMergedAssemblyReferencesMap != null);
        ...
        {
            RoslynDebug.Assert(_isBound == 0);
            RoslynDebug.Assert(_lazyHasCircularReference == ThreeState.Unknown);
            RoslynDebug.Assert(_lazyReferencedAssembliesMap == null);
            RoslynDebug.Assert(_lazyReferencedModuleIndexMap == null);
            RoslynDebug.Assert(_lazyReferenceDirectiveMap == null);
            RoslynDebug.Assert(_lazyDirectiveReferences.IsDefault);
            RoslynDebug.Assert(_lazyImplicitReferenceResolutions == null);
            RoslynDebug.Assert(_lazyExplicitReferences.IsDefault);
            RoslynDebug.Assert(_lazyReferencedModules.IsDefault);
            RoslynDebug.Assert(_lazyReferencedModulesReferences.IsDefault);
            RoslynDebug.Assert(_lazyReferencedAssemblies.IsDefault);
            RoslynDebug.Assert(_lazyAliasesOfReferencedAssemblies.IsDefault);
            RoslynDebug.Assert(_lazyMergedAssemblyReferencesMap == null);
            RoslynDebug.Assert(_lazyUnifiedAssemblies.IsDefault);
            RoslynDebug.Assert(_lazyCorLibraryOpt == null);
        }
        ...
        internal void AssertBound()
        {
            ...
            RoslynDebug.Assert(_lazyReferencedAssemblies.Length == 0 || _lazyCorLibraryOpt != null);
        }

        [Conditional("DEBUG")]
        internal void AssertCanReuseForCompilation(TCompilation compilation)
        {
            RoslynDebug.Assert(compilation.MakeSourceAssemblySimpleName() == this.SimpleAssemblyName);
        }
            ...
            Dictionary<MetadataReference, ImmutableArray<MetadataReference>>? mergedAssemblyReferencesMapOpt)
        {
            ...
            RoslynDebug.Assert(referencedModules.Length == referencedModulesReferences.Length);
            RoslynDebug.Assert(referencedModules.Length == referencedModulesMap.Count);
            RoslynDebug.Assert(referencedAssemblies.Length == aliasesOfReferencedAssemblies.Length);
            ...
            out Dictionary<MetadataReference, ImmutableArray<MetadataReference>>? mergedAssemblyReferencesMapOpt)
        {
            ...
            for (int i = 0; i < referenceMap.Length; i++)
            {
                ...
                else
                {
                    ...
                    RoslynDebug.Assert(aliasesOfReferencedAssembliesBuilder.Count == assemblyIndex);
            ...
            RoslynDebug.Assert(!aliasesOfReferencedAssembliesBuilder.Any(a => a.IsDefault));
        ...
        {
            RoslynDebug.Assert(originalIdentities.Length == symbols.Length);
            ...
            for (int i = 0; i < originalIdentities.Length; i++)
            {
                ...
                {
                    RoslynDebug.Assert(versionPattern.Build == ushort.MaxValue || versionPattern.Revision == ushort.MaxValue);
                ...
                else
                {
                    ...
                    RoslynDebug.Assert(originalIdentity == symbolIdentity);
                }
            }
        ...
        internal static bool CompareVersionPartsSpecifiedInSource(Version version, Version candidateVersion, TAssemblySymbol candidateSymbol)
        {
            ...
            RoslynDebug.Assert(versionPattern is null || versionPattern.Build == ushort.MaxValue || versionPattern.Revision == ushort.MaxValue);
            ...
            ArrayBuilder<ImmutableArray<string>> aliasesOfReferencedAssembliesBuilder)
        {
            ...
            RoslynDebug.Assert(bindingResult.Length == aliasesOfReferencedAssembliesBuilder.Count + 1);
            ...
            foreach (ResolvedReference reference in referenceMap)
            {
                if (!reference.IsSkipped && !reference.RecursiveAliasesOpt.IsDefault)
                {
                    ...
                    RoslynDebug.Assert(reference.Kind == MetadataImageKind.Assembly);
                    visitedAssemblies.Clear();

                    RoslynDebug.Assert(assemblyIndicesToProcess.Count == 0);
                    ...
                    while (assemblyIndicesToProcess.Count > 0)
                    {
                        ...
                        RoslynDebug.Assert(referenceBinding is object);
