...
namespace Loretta.CodeAnalysis
{
    ...
    internal sealed class SecurityWellKnownAttributeData
    {
        ...
        {
            RoslynDebug.Assert(attributeIndex >= 0 && attributeIndex < totalSourceAttributes);
            RoslynDebug.Assert(action != 0);
            ...
            RoslynDebug.Assert(_lazySecurityActions.Length == totalSourceAttributes);
        ...
        {
            RoslynDebug.Assert(attributeIndex >= 0 && attributeIndex < totalSourceAttributes);
            RoslynDebug.Assert(resolvedFilePath != null);
            ...
            RoslynDebug.Assert(_lazyPathsForPermissionSetFixup.Length == totalSourceAttributes);
        ...
        {
            RoslynDebug.Assert(!customAttributes.IsDefault);
            RoslynDebug.Assert(_lazyPathsForPermissionSetFixup == null || _lazySecurityActions != null && _lazyPathsForPermissionSetFixup.Length == _lazySecurityActions.Length);

            if (_lazySecurityActions != null)
            {
                RoslynDebug.Assert(_lazySecurityActions != null);
                RoslynDebug.Assert(_lazySecurityActions.Length == customAttributes.Length);
