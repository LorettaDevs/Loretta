...
namespace Loretta.CodeAnalysis
{
    public abstract class AttributeData
    {
        ...
        internal ConstantValue DecodeDecimalConstantValue()
        {
            ...
            RoslynDebug.Assert(AttributeConstructor is object);
            var parameters = AttributeConstructor.Parameters;
            ImmutableArray<TypedConstant> args = this.CommonConstructorArguments;

            RoslynDebug.Assert(parameters.Length == 5);
            RoslynDebug.Assert(parameters[0].Type.SpecialType == SpecialType.System_Byte);
            RoslynDebug.Assert(parameters[1].Type.SpecialType == SpecialType.System_Byte);
            ...
            {
                RoslynDebug.Assert(parameters[2].Type.SpecialType == SpecialType.System_Int32);
                RoslynDebug.Assert(parameters[3].Type.SpecialType == SpecialType.System_Int32);
                RoslynDebug.Assert(parameters[4].Type.SpecialType == SpecialType.System_Int32);
            ...
            {
                RoslynDebug.Assert(parameters[2].Type.SpecialType == SpecialType.System_UInt32);
                RoslynDebug.Assert(parameters[3].Type.SpecialType == SpecialType.System_UInt32);
                RoslynDebug.Assert(parameters[4].Type.SpecialType == SpecialType.System_UInt32);
        ...
        private ObsoleteAttributeData DecodeObsoleteAttribute()
        {
            ...
            if (args.Length > 0)
            {
                ...
                RoslynDebug.Assert(args.Length <= 2);
                message = (string?)args[0].ValueInternal;

                if (args.Length == 2)
                {
                    RoslynDebug.Assert(args[1].ValueInternal is object);
        ...
        private ObsoleteAttributeData DecodeExperimentalAttribute()
        {
            ...
            RoslynDebug.Assert(this.CommonConstructorArguments.Length == 0);
        ...
        {
            RoslynDebug.Assert(arguments.AttributeSyntaxOpt is object);

            MethodImplOptions options;
            var attribute = arguments.Attribute;
            if (attribute.CommonConstructorArguments.Length == 1)
            {
                RoslynDebug.Assert(attribute.AttributeConstructor is object);
            ...
            where TAttributeData : AttributeData
        {
            ...
            foreach (var namedArg in attribute.CommonNamedArguments)
            {
                switch (namedArg.Key)
                {
                    ...
                    case "Size":
                        size = namedArg.Value.DecodeValue<int>(Loretta.CodeAnalysis.SpecialType.System_Int32);
                        if (size < 0)
                        {
                            messageProvider.ReportInvalidNamedArgument(arguments.Diagnostics, arguments.AttributeSyntaxOpt, position, attribute.AttributeClass, namedArg.Key);
                            hasErrors = true;
        ...
        internal static AttributeUsageInfo DecodeAttributeUsageAttribute(TypedConstant positionalArg, ImmutableArray<KeyValuePair<string, TypedConstant>> namedArgs)
        {
            ...
            RoslynDebug.Assert(positionalArg.ValueInternal is object);
