...
#nullable enable
        public static RuleSet LoadFromFile(string filePath)
        {
            ...
            using (XmlReader xmlReader = XmlReader.Create(stream, settings))
            {
                ...
                RoslynDebug.Assert(nodeList.Count == 1, "Multiple top-level nodes!");
                RoslynDebug.Assert(nodeList[0].Name == RuleSetNodeName);
        ...
        private static List<KeyValuePair<string, ReportDiagnostic>> ReadRules(XElement rulesNode)
        {
            ...
            foreach (XElement ruleNode in rulesNode.Elements())
            {
                ...
                else
                {
                    ...
                    RoslynDebug.Assert(false, "Unknown child node in Rules node");
                }
            }
