...
namespace Loretta.CodeAnalysis
{
    ...
    public sealed partial class AnalyzerConfig
    {
        ...
        /// </summary>
        internal bool IsRoot => GlobalSection.Properties.TryGetValue("root", out var val) && val == "true";
        ...
        internal int GlobalLevel
        {
            get
            {
                if (GlobalSection.Properties.TryGetValue(GlobalLevelKey, out var val) && int.TryParse(val, out var level))
                {
                    return level;
                }
            ...
            string pathToFile)
        {
            ...
            var directory = Path.GetDirectoryName(pathToFile) ?? pathToFile;
        ...
        public static AnalyzerConfig Parse(SourceText text, string? pathToFile)
        {
            ...
            foreach (var textLine in text.Lines)
            {
                ...
                if (sectionMatches.Count > 0 && sectionMatches[0].Groups.Count > 0)
                {
                    ...
                    RoslynDebug.Assert(!string.IsNullOrEmpty(sectionName));
                ...
                if (propMatches.Count > 0 && propMatches[0].Groups.Count > 1)
                {
                    ...
                    RoslynDebug.Assert(!string.IsNullOrEmpty(key));
                    RoslynDebug.Assert(key == key.Trim());
                    RoslynDebug.Assert(value == value?.Trim());
