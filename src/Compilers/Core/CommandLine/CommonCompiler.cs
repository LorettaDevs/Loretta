...
namespace Loretta.CodeAnalysis
{
    ...
    internal abstract partial class CommonCompiler
    {
        ...
        public CommonCompiler(CommandLineParser parser, string? responseFile, string[] args, BuildPaths buildPaths, string? additionalReferenceDirectories, IAnalyzerAssemblyLoader assemblyLoader)
        {
            ...
            RoslynDebug.Assert(null == responseFile || PathUtilities.IsAbsolute(responseFile));
            ...
            [NotNullWhen(true)] out AnalyzerConfigSet? analyzerConfigSet)
        {
            ...
            foreach (var configPath in analyzerConfigPaths)
            {
                ...
                RoslynDebug.Assert(normalizedPath is object);
        ...
        {
            RoslynDebug.Assert(compilation.Options.SourceReferenceResolver is object);
            ...
            foreach (var path in embeddedFileOrderedSet)
            {
                ...
                else
                {
                    ...
                    RoslynDebug.Assert(text != null || diagnostics.HasAnyErrors());
                }
        ...
        internal bool ReportDiagnostics(IEnumerable<Diagnostic> diagnostics, TextWriter consoleOutput, ErrorLogger? errorLoggerOpt, Compilation? compilation)
        {
            ...
            void reportDiagnostic(Diagnostic diag, SuppressionInfo? suppressionInfo)
            {
                if (_reportedDiagnostics.Contains(diag))
                {
                    ...
                    //Loretta.Utilities.RoslynDebug.Assert(diag.Severity != DiagnosticSeverity.Error);
                    return;
                }
        ...
        {
            RoslynDebug.Assert(Arguments.ErrorLogOptions?.Path != null);
            ...
            if (errorLog == null)
            {
                RoslynDebug.Assert(diagnostics.HasAnyErrors());
                logger = null;
            }
            else
            {
        ...
        {
            RoslynDebug.Assert(!Arguments.IsScriptRunner);
            ...
            if (reportAnalyzer)
            {
                RoslynDebug.Assert(analyzerDriver is object);
            ...
            ImmutableArray<AnalyzerConfigOptionsResult> additionalFileOptions = default)
        {
            ...
            foreach (var syntaxTree in syntaxTrees)
            {
                ...
                if (options.Count > 0)
                {
                    RoslynDebug.Assert(existing.GetOptions(syntaxTree) == CompilerAnalyzerConfigOptions.Empty);
            ...
            if (!additionalFiles.IsDefault)
            {
                for (i = 0; i < additionalFiles.Length; i++)
                {
                    ...
                    if (options.Count > 0)
                    {
                        RoslynDebug.Assert(existing.GetOptions(additionalFiles[i]) == CompilerAnalyzerConfigOptions.Empty);
            ...
            out AnalyzerDriver? analyzerDriver)
        {
            ...
            if (!analyzers.IsEmpty || !generators.IsEmpty)
            {
                ...
                if (Arguments.AnalyzerConfigPaths.Length > 0)
                {
                    RoslynDebug.Assert(analyzerConfigSet is object);
                ...
                if (!generators.IsEmpty)
                {
                    ...
                    try
                    {
                        ...
                        {
                            RoslynDebug.Assert(!string.IsNullOrWhiteSpace(tree.FilePath));
                            cancellationToken.ThrowIfCancellationRequested();
                            ...
                            if (hasGeneratedOutputPath)
                            {
                                ...
                                {
                                    RoslynDebug.Assert(tree.Encoding is object);
        ...
        private bool WriteTouchedFiles(DiagnosticBag diagnostics, TouchedFileLogger? touchedFilesLogger, string? finalXmlFilePath)
        {
            ...
            {
                RoslynDebug.Assert(touchedFilesLogger != null);
        ...
        {
            RoslynDebug.Assert(analyzerDriver.AnalyzerExecutionTimes != null);
