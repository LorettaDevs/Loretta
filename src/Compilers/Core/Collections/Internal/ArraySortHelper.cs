...
namespace Loretta.CodeAnalysis.Collections.Internal
{
        ...
        where T : IComparable<T>
    {
        ...
        {
            RoslynDebug.Assert(!Unsafe.AreSame(ref i, ref j));
    ...
    internal static class SegmentedArraySortHelper<TKey, TValue>
    {
        ...
        {
            RoslynDebug.Assert(comparer != null);
            RoslynDebug.Assert(0 <= i && i < keys.Length && i < values.Length);
            RoslynDebug.Assert(0 <= j && j < keys.Length && j < values.Length);
            RoslynDebug.Assert(i != j);
        ...
        {
            RoslynDebug.Assert(comparer != null);
            RoslynDebug.Assert(keys.Length == values.Length);
        ...
        {
            RoslynDebug.Assert(keys.Length > 0);
            RoslynDebug.Assert(values.Length == keys.Length);
            RoslynDebug.Assert(depthLimit >= 0);
            RoslynDebug.Assert(comparer != null);
