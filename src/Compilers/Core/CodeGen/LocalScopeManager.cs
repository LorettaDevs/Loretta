    ...
    internal partial class ILBuilder
    {
        private sealed class LocalScopeManager
        {
            ...
            internal ScopeInfo OpenScope(ScopeType scopeType, Cci.ITypeReference exceptionType)
            {
                ...
                RoslynDebug.Assert(_enclosingExceptionHandler == GetEnclosingExceptionHandler());
                return scope;
            }
        ...
        internal abstract class ScopeInfo
        {
            ...
            public virtual ScopeInfo OpenScope(ScopeType scopeType,
                Loretta.Cci.ITypeReference exceptionType,
                ExceptionHandlerScope currentHandler)
            {
                ...
                {
                    RoslynDebug.Assert(scopeType == ScopeType.Variable || scopeType == ScopeType.StateMachineVariable);
            ...
            {
                RoslynDebug.Assert(scopes.Count > 0);
        ...
        internal class LocalScopeInfo : ScopeInfo
        {
            ...
            internal void AddLocalConstant(LocalConstantDefinition constant)
            {
                ...
                RoslynDebug.Assert(constant.Name != null);
            ...
            internal void AddUserHoistedLocal(int slotIndex)
            {
                ...
                RoslynDebug.Assert(slotIndex >= 0);
        ...
        internal sealed class ExceptionHandlerScope : LocalScopeInfo
        {
            ...
            private readonly Loretta.Cci.ITypeReference _exceptionType;
            ...
            public ExceptionHandlerScope(ExceptionHandlerContainerScope containingScope, ScopeType type, Loretta.Cci.ITypeReference exceptionType)
            {
                RoslynDebug.Assert((type == ScopeType.Try) || (type == ScopeType.Catch) || (type == ScopeType.Filter) || (type == ScopeType.Finally) || (type == ScopeType.Fault));
                RoslynDebug.Assert((type == ScopeType.Catch) == (exceptionType != null));
            ...
            public Loretta.Cci.ITypeReference ExceptionType => _exceptionType;
            ...
            {
                RoslynDebug.Assert(_type == ScopeType.Filter);
                RoslynDebug.Assert(_lastFilterConditionBlock == null);
            ...
            public override void ClosingScope(ILBuilder builder)
            {
                switch (_type)
                {
                    ...
                    default:
                        ...
                        RoslynDebug.Assert(endLabel != null);
            ...
            {
                RoslynDebug.Assert(LeaderBlock != null);
            }

            public override BasicBlock CreateBlock(ILBuilder builder)
            {
                RoslynDebug.Assert(builder.EnclosingExceptionHandler == this);
        ...
        internal sealed class ExceptionHandlerContainerScope : ScopeInfo
        {
            ...
            public override ScopeInfo OpenScope(ScopeType scopeType,
                ...
                ExceptionHandlerScope currentExceptionHandler)
            {
                RoslynDebug.Assert(((_handlers.Count == 0) && (scopeType == ScopeType.Try)) ||
                ...
                RoslynDebug.Assert(currentExceptionHandler == _containingHandler);
            ...
            public override void CloseScope(ILBuilder builder)
            {
                RoslynDebug.Assert(_handlers.Count > 1);
                ...
                RoslynDebug.Assert(builder._currentBlock == builder._labelInfos[_endLabel].bb);
            ...
            internal override void GetExceptionHandlerRegions(ArrayBuilder<Cci.ExceptionHandlerRegion> regions)
            {
                ...
                foreach (var handlerScope in _handlers)
                {
                    ...
                    if (tryScope == null)
                    {
                        ...
                        RoslynDebug.Assert(handlerScope.Type == ScopeType.Try);

                        tryScope = handlerScope;
                        tryBounds = handlerBounds;

                        var reachability = tryScope.LeaderBlock.Reachability;
                        RoslynDebug.Assert((reachability == Reachability.Reachable) || (reachability == Reachability.NotReachable));

                        // All handler blocks should have same reachability.
                        RoslynDebug.Assert(_handlers.All(h => (h.LeaderBlock.Reachability == reachability)));
        ...
        internal struct ScopeBounds
        {
            ...
            {
                RoslynDebug.Assert(begin >= 0 && end >= 0);
                this.Begin = begin;
                this.End = end;
            }
        }
