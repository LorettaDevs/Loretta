...
namespace Loretta.CodeAnalysis.CodeGen
{
    ...
    internal sealed partial class ILBuilder
    {
        ...
        {
            RoslynDebug.Assert(!_pendingBlockCreate || (_currentBlock == null));
        ...
        {
            RoslynDebug.Assert(_currentBlock == null);
        ...
        {
            _currentBlock = block;
            RoslynDebug.Assert(_lastCompleteBlock.NextBlock == null);
        ...
        private void CreateBlockIfPending()
        {
            if (_pendingBlockCreate)
            {
                ...
                RoslynDebug.Assert(_currentBlock != null);
                RoslynDebug.Assert(!_pendingBlockCreate);
            }
        }
        ...
        private void ReconcileTrailingMarkers()
        {
                ...
                _allocatedILMarkers[_lastCompleteBlock.LastILMarker].BlockOffset == _lastCompleteBlock.RegularInstructionsLength)
            {
                ...
                while (_lastCompleteBlock.LastILMarker >= 0 &&
                      _allocatedILMarkers[_lastCompleteBlock.LastILMarker].BlockOffset == _lastCompleteBlock.RegularInstructionsLength)
                {
                    RoslynDebug.Assert((startMarker < 0) || (startMarker == (_lastCompleteBlock.LastILMarker + 1)));
        ...
        {
            RoslynDebug.Assert(AllBlocks(block => (block.Reachability == Reachability.NotReachable)));
...
tryAgain:
        ...
        private static void MarkReachableFromTry(ArrayBuilder<BasicBlock> reachableBlocks, BasicBlock block)
        {
            ...
            RoslynDebug.Assert(handlerBlock != null);

            // Subsequent handlers are either one or more catch
            // blocks or a single finally block, but not both.
            if (handlerBlock.Type == BlockType.Finally)
            {
                RoslynDebug.Assert(handlerBlock.NextExceptionHandler == null);
            ...
            else
            {
                ...
                {
                    RoslynDebug.Assert(handlerBlock.Type == BlockType.Catch || handlerBlock.Type == BlockType.Fault || handlerBlock.Type == BlockType.Filter);
                    PushReachableBlockToProcess(reachableBlocks, handlerBlock);
        ...
        private bool ForwardLabelsNoLeaving()
        {
            ...
            do
            {
                ...
                foreach (object label in labels)
                {
                    ...
                    RoslynDebug.Assert(!IsSpecialEndHandlerBlock(targetBlock));
        ...
        private bool DropUnreachableBlocks()
        {
            ...
            RoslynDebug.Assert(AllBlocks(block => (block.Reachability == Reachability.Reachable) || (block.Reachability == Reachability.BlockedByFinally)));
        ...
        private void RewriteSpecialBlocks()
        {
            ...
            while (current != null)
            {
                ...
                RoslynDebug.Assert(current.Reachability != Reachability.BlockedByFinally ||
            ...
            RoslynDebug.Assert(AllBlocks(block => !IsSpecialEndHandlerBlock(block)));
        }
        ...
        private static bool IsSpecialEndHandlerBlock(BasicBlock block)
        {
            ...
            RoslynDebug.Assert(block.BranchBlock == block);
            return true;
        }
        ...
        private void RealizeBlocks()
        {
            ...
            for (var block = leaderBlock; block != null; block = block.NextBlock)
            {
                ...
                if (blockFirstMarker >= 0)
                {
                    ...
                    RoslynDebug.Assert(blockLastMarker >= blockFirstMarker);
                    ...
                    default:
                        ...
                        if (block.BranchLabel != null)
                        {
                            ...
                            if (block.BranchCode.GetBranchOperandSize() == 1)
                            {
                                ...
                                RoslynDebug.Assert(btOffset == offset);
            ...
            if (this.SeqPointsOpt != null)
            {
                ...
                foreach (var seqPoint in this.SeqPointsOpt)
                {
                    ...
                    if (offset >= 0)
                    {
                        ...
                        {
                            RoslynDebug.Assert(lastOffset < offset);
                            // if there are any sequence points, there must
                            // be a sequence point at offset 0.
                            RoslynDebug.Assert((lastOffset >= 0) || (offset == 0));
        ...
        internal void DefineSequencePoint(SyntaxTree syntaxTree, TextSpan span)
        {
            ...
            {
                RoslynDebug.Assert(this.SeqPointsOpt.Count == 0);
        ...
        {
            RoslynDebug.Assert(_initialHiddenSequencePointMarker < 0);
        ...
        {
            RoslynDebug.Assert(_emitState.CurStack == 0);
        }

        // true if there may have been a label generated with no subsequent code
        internal bool IsJustPastLabel()
        {
            RoslynDebug.Assert(_emitState.InstructionsEmitted >= _instructionCountAtLastLabel);
        ...
        {
            RoslynDebug.Assert(this.RealizedIL.IsDefault, "Too late to allocate a new IL marker");
            if (_allocatedILMarkers == null)
            {
            ...
            RoslynDebug.Assert(curBlock != null);
        ...
        {
            RoslynDebug.Assert(!RealizedIL.IsDefault, "Builder must be realized to perform this operation");
            RoslynDebug.Assert(_allocatedILMarkers != null, "There are not markers in this builder");
            RoslynDebug.Assert(ilMarker >= 0 && ilMarker < _allocatedILMarkers.Count, "Wrong builder?");
