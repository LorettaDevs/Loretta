...
namespace Loretta.CodeAnalysis.CodeGen
{
    ...
    internal class PermissionSetAttributeWithFileReference : Cci.ICustomAttribute
    {
        ...
        public ImmutableArray<Cci.IMetadataNamedArgument> GetNamedArguments(EmitContext context)
        {
            ...
            RoslynDebug.Assert(namedArgs.Length == 1);

            // Named argument must be 'File' property of string type
            var fileArg = namedArgs.First();
            RoslynDebug.Assert(fileArg.ArgumentName == FilePropertyName);
            RoslynDebug.Assert(context.Module.IsPlatformType(fileArg.Type, Cci.PlatformType.SystemString));

            // Named argument value must be a non-empty string
            RoslynDebug.Assert(fileArg.ArgumentValue is MetadataConstant);
            var fileName = (string?)((MetadataConstant)fileArg.ArgumentValue).Value;
            RoslynDebug.Assert(!String.IsNullOrEmpty(fileName));
            ...
            RoslynDebug.Assert(((INamedTypeSymbol)iSymbol).GetMembers(HexPropertyName).Any(
        ...
        public ushort NamedArgumentCount
        {
            get
            {
                RoslynDebug.Assert(_sourceAttribute.NamedArgumentCount == 1);
                return 1;
            }
        }
