...
namespace Loretta.CodeAnalysis.CodeGen
{
    internal partial class ILBuilder
    {
        ...
        {
            RoslynDebug.Assert(!code.IsControlTransfer(),
        ...
        internal void MarkLabel(object label)
        {
            ...
            {
                RoslynDebug.Assert(labelInfo.bb == null, "duplicate use of a label");
                ...
                RoslynDebug.Assert(curStack == labelStack, "forward branches and fall-through must agree on stack depth");
        ...
        internal void EmitBranch(ILOpCode code, object label, ILOpCode revOpCode = ILOpCode.Nop)
        {
            ...
            RoslynDebug.Assert(validOpCode);
            RoslynDebug.Assert(revOpCode == ILOpCode.Nop || revOpCode.IsBranch());
            RoslynDebug.Assert(!code.HasVariableStackBehavior());
            ...
            {
                RoslynDebug.Assert(labelInfo.stack == _emitState.CurStack, "branches to same label with different stacks");
            }
            ...
            RoslynDebug.Assert((code != ILOpCode.Nop) || (block == _labelInfos[label].bb));
        ...
        {
            RoslynDebug.Assert(caseLabels.Length > 0);
        ...
        {
            RoslynDebug.Assert(caseLabels.Length > 0);
            RoslynDebug.Assert(keyTypeCode != Cci.PrimitiveTypeCode.String);
        ...
        internal void EmitSwitch(object[] labels)
        {
            ...
            foreach (object label in labels)
            {
                ...
                {
                    RoslynDebug.Assert(ld.stack == curStack, "branches to same label with different stacks");
        ...
        internal void EmitRet(bool isVoid)
        {
            ...
            RoslynDebug.Assert(!this.InExceptionHandler);
        ...
        /// </summary>
        internal void EmitArrayCreation(Loretta.Cci.IArrayTypeReference arrayType, SyntaxNode syntaxNode, DiagnosticBag diagnostics)
        {
            RoslynDebug.Assert(!arrayType.IsSZArray, "should be used only with multidimensional arrays");
        ...
        /// </summary>
        internal void EmitArrayElementLoad(Loretta.Cci.IArrayTypeReference arrayType, SyntaxNode syntaxNode, DiagnosticBag diagnostics)
        {
            RoslynDebug.Assert(!arrayType.IsSZArray, "should be used only with multidimensional arrays");
        ...
        /// </summary>
        internal void EmitArrayElementAddress(Loretta.Cci.IArrayTypeReference arrayType, SyntaxNode syntaxNode, DiagnosticBag diagnostics)
        {
            RoslynDebug.Assert(!arrayType.IsSZArray, "should be used only with multidimensional arrays");
