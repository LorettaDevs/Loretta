...
namespace Loretta.CodeAnalysis.CodeGen
{
    internal partial struct SwitchIntegralJumpTableEmitter
    {
        private struct SwitchBucket
        {
            ...
            {
                RoslynDebug.Assert((uint)startIndex < (uint)endIndex);
            ...
            {
                RoslynDebug.Assert((uint)startIndex <= (uint)endIndex);
                RoslynDebug.Assert((uint)startIndex != (uint)endIndex || isDegenerate);
            ...
            internal KeyValuePair<ConstantValue, object> this[int i]
            {
                get
                {
                    RoslynDebug.Assert(i < LabelsCount, "index out of range");
            ...
            internal int DegenerateBucketSplit
            {
                get
                {
                    ...
                    RoslynDebug.Assert(_startLabelIndex != _endLabelIndex, "1-sized buckets should be already known as degenerate.");
            ...
            {
                RoslynDebug.Assert(!BucketOverflowUInt64Limit(startConstant, endConstant));
                RoslynDebug.Assert(endConstant.Discriminator == startConstant.Discriminator);

                ulong bucketSize;

                if (startConstant.IsNegativeNumeric || endConstant.IsNegativeNumeric)
                {
                    RoslynDebug.Assert(endConstant.Int64Value >= startConstant.Int64Value);
                    bucketSize = unchecked((ulong)(endConstant.Int64Value - startConstant.Int64Value + 1));
                }
                else
                {
                    RoslynDebug.Assert(endConstant.UInt64Value >= startConstant.UInt64Value);
            ...
            {
                RoslynDebug.Assert(IsValidSwitchBucketConstantPair(startConstant, endConstant));
            ...
            {
                RoslynDebug.Assert(prevBucket._endLabelIndex + 1 == _startLabelIndex);
