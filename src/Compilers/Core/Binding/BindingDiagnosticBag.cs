...
namespace Loretta.CodeAnalysis
{
    ...
    internal abstract class BindingDiagnosticBag
    {
        ...
        {
            RoslynDebug.Assert(DiagnosticBag is object);
        ...
        where TAssemblySymbol : class, IAssemblySymbolInternal
    {
        ...
        protected BindingDiagnosticBag(DiagnosticBag? diagnosticBag, ICollection<TAssemblySymbol>? dependenciesBag)
            : base(diagnosticBag)
        {
            RoslynDebug.Assert(diagnosticBag?.GetType().IsValueType != true);
        ...
        internal void AddRange(ImmutableBindingDiagnostic<TAssemblySymbol> other, bool allowMismatchInDependencyAccumulation = false)
        {
            ...
            RoslynDebug.Assert(allowMismatchInDependencyAccumulation || other.Dependencies.IsDefaultOrEmpty || this.AccumulatesDependencies || !this.AccumulatesDiagnostics);
        ...
        internal void AddRange(BindingDiagnosticBag<TAssemblySymbol>? other, bool allowMismatchInDependencyAccumulation = false)
        {
            if (other is object)
            {
                ...
                RoslynDebug.Assert(allowMismatchInDependencyAccumulation || !other.AccumulatesDependencies || this.AccumulatesDependencies);
        ...
        {
            RoslynDebug.Assert(allowMismatchInDependencyAccumulation || !dependencies.AccumulatesDependencies || this.AccumulatesDependencies);
        ...
        {
            RoslynDebug.Assert(!useSiteInfo.AccumulatesDependencies || this.AccumulatesDependencies);
            if (DependenciesBag is object)
            {
