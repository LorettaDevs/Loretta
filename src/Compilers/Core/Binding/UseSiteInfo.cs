...
namespace Loretta.CodeAnalysis
{
    ...
    internal readonly struct UseSiteInfo<TAssemblySymbol> where TAssemblySymbol : class, IAssemblySymbolInternal
    {
        ...
        {
            RoslynDebug.Assert(diagnosticInfo?.Severity != DiagnosticSeverity.Error || (primaryDependency is null && secondaryDependencies?.IsEmpty != false));
            RoslynDebug.Assert(primaryDependency is null || primaryDependency != primaryDependency.CorLibrary);
            RoslynDebug.Assert(secondaryDependencies?.IsEmpty != false || !secondaryDependencies.Any(dependency => dependency == dependency.CorLibrary));
        ...
        public UseSiteInfo<TAssemblySymbol> AdjustDiagnosticInfo(DiagnosticInfo? diagnosticInfo)
        {
            if ((object?)DiagnosticInfo != diagnosticInfo)
            {
                ...
                {
                    RoslynDebug.Assert(DiagnosticInfo?.Severity != DiagnosticSeverity.Error);
                }
    ...
    internal struct CompoundUseSiteInfo<TAssemblySymbol> where TAssemblySymbol : class, IAssemblySymbolInternal
    {
        ...
        {
            RoslynDebug.Assert(assemblyBeingBuilt is object);
            RoslynDebug.Assert(assemblyBeingBuilt is ISourceAssemblySymbolInternal);
        ...
        {
            RoslynDebug.Assert(discardLevel != DiscardLevel.None);
            this = default;
            ...
            get
            {
                ...
                switch (_discardLevel)
                {
                    case DiscardLevel.DiagnosticsAndDependencies:
                        RoslynDebug.Assert(_diagnostics is null);
                        RoslynDebug.Assert(_dependencies is null);
                        RoslynDebug.Assert(!_hasErrors);
                        break;
        ...
        public IReadOnlyCollection<DiagnosticInfo>? Diagnostics
        {
            get
            {
                RoslynDebug.Assert(AccumulatesDiagnostics);
                AssertInternalConsistency();
                return _diagnostics;
            }
        }
        ...
        {
            RoslynDebug.Assert(_discardLevel switch { DiscardLevel.None => true, DiscardLevel.Dependencies => true, DiscardLevel.DiagnosticsAndDependencies => true, _ => false });
            RoslynDebug.Assert(_hasErrors == (_diagnostics?.Any(d => d.Severity == DiagnosticSeverity.Error) ?? false));
            RoslynDebug.Assert(!_hasErrors || (_dependencies is null));
        }
        ...
        public IReadOnlyCollection<TAssemblySymbol>? Dependencies
        {
            get
            {
                RoslynDebug.Assert(AccumulatesDependencies);
                AssertInternalConsistency();
        ...
        {
            RoslynDebug.Assert(!info.AccumulatesDependencies || this.AccumulatesDependencies);
            if (!_hasErrors && AccumulatesDependencies)
            {
        ...
        {
            RoslynDebug.Assert(other.AccumulatesDiagnostics);
            RoslynDebug.Assert(other._discardLevel != DiscardLevel.None || _discardLevel == DiscardLevel.None);
    ...
    internal struct CachedUseSiteInfo<TAssemblySymbol> where TAssemblySymbol : class, IAssemblySymbolInternal
    {
        ...
        {
            RoslynDebug.Assert(diagnosticInfo is null || diagnosticInfo.Severity == DiagnosticSeverity.Error);
        ...
        private static ImmutableHashSet<TAssemblySymbol> GetDependenciesToCache(TAssemblySymbol? primaryDependency, UseSiteInfo<TAssemblySymbol> useSiteInfo)
        {
            ...
            RoslynDebug.Assert(primaryDependency is object || (useSiteInfo.PrimaryDependency is null && secondaryDependencies.IsEmpty));
            RoslynDebug.Assert(primaryDependency == useSiteInfo.PrimaryDependency || useSiteInfo.DiagnosticInfo?.Severity == DiagnosticSeverity.Error);
        ...
        {
            object? info = Compact(value.DiagnosticInfo, GetDependenciesToCache(primaryDependency, value));
            RoslynDebug.Assert(info is object);
        ...
        private class Boxed
        {
            ...
            {
                RoslynDebug.Assert(!dependencies.IsEmpty);
                DiagnosticInfo = diagnosticInfo;
