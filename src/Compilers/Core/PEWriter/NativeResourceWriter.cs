        ...
        public static void SerializeWin32Resources(BlobBuilder builder, IEnumerable<IWin32Resource> theResources, int resourcesRva)
        {
            ...
            foreach (IWin32Resource r in theResources)
            {
                ...
                if (typeDifferent)
                {
                    ...
                    if (lastTypeID < 0)
                    {
                        RoslynDebug.Assert(typeDirectory.NumberOfIdEntries == 0, "Not all Win32 resources with types encoded as strings precede those encoded as ints");
                ...
                if (typeDifferent || (r.Id < 0 && r.Name != lastName) || r.Id > lastID)
                {
                    ...
                    if (lastID < 0)
                    {
                        RoslynDebug.Assert(nameDirectory.NumberOfIdEntries == 0, "Not all Win32 resources with names encoded as strings precede those encoded as ints");
