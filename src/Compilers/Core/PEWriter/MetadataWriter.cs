...
using Loretta.CodeAnalysis.Emit;
using Loretta.CodeAnalysis.PooledObjects;
using Loretta.CodeAnalysis.Symbols;
using Loretta.DiaSymReader;
...
namespace Loretta.Cci
{
    internal abstract partial class MetadataWriter
    {
        ...
        {
            RoslynDebug.Assert(metadata != debugMetadataOpt);
        ...
        public static IUnitReference GetDefiningUnitReference(ITypeReference typeReference, EmitContext context)
        {
            ...
            RoslynDebug.Assert(namespaceTypeReference.AsGenericTypeInstanceReference == null);
        ...
        {
            RoslynDebug.Assert(!_tableIndicesAreComplete);
        ...
        private BlobHandle GetMethodSignatureHandleAndBlob(IMethodReference methodReference, out ImmutableArray<byte> signatureBlob)
        {
            ...
            RoslynDebug.Assert((methodReference.CallingConvention & CallingConvention.Generic) != 0 == (methodReference.GenericParameterCount > 0));
        ...
        {
            RoslynDebug.Assert(!(signature is IMethodReference));
            var builder = PooledBlobBuilder.GetInstance();
        ...
        {
            RoslynDebug.Assert(str != null); // No need to handle in an internal utility.
        ...
        {
            RoslynDebug.Assert(nativePdbWriterOpt == null ^ portablePdbStreamOpt == null);
            ...
            // stream.
            RoslynDebug.Assert(module.SerializationProperties.PersistentIdentifier != default(Guid));
            ...
            RoslynDebug.Assert(mappedFieldDataBuilder.Count == 0);
            RoslynDebug.Assert(managedResourceDataBuilder.Count == 0);
            RoslynDebug.Assert(mvidFixup.IsDefault);
            RoslynDebug.Assert(mvidStringFixup.IsDefault);
        ...
        {
            RoslynDebug.Assert(typeSystemRowCounts[(int)TableIndex.EncLog] == 0);
            RoslynDebug.Assert(typeSystemRowCounts[(int)TableIndex.EncMap] == 0);
        ...
        private void PopulateAssemblyTableRows()
        {
            ...
            RoslynDebug.Assert(sourceAssembly != null);
        ...
        {
            RoslynDebug.Assert(this.IsFullMetadata); // parentToken is not relative
        ...
        {
            RoslynDebug.Assert(this.IsFullMetadata);
        ...
        private void PopulateExportedTypeTableRows()
        {
            ...
            foreach (var exportedType in exportedTypes)
            {
                ...
                {
                    RoslynDebug.Assert(exportedType.ParentIndex != -1);
        ...
        private void PopulateManifestResourceTableRows(BlobBuilder resourceDataWriter, BlobBuilder dynamicAnalysisDataOpt)
        {
            ...
            RoslynDebug.Assert((resourceDataWriter.Count % ManagedPEBuilder.ManagedResourcesDataAlignment) == 0);
        }
        ...
        {
            RoslynDebug.Assert(MetadataOnly);
                    ...
                    case OperandType.InlineSig:
                        {
                            ...
                            if (operandType == OperandType.InlineTok)
                            {
                                ...
                                {
                                    RoslynDebug.Assert(ReadByte(generatedIL, offset - 1) == (byte)ILOpCode.Ldtoken);
                    ...
                    case OperandType.InlineString:
                        {
                            ...
                            if ((uint)pseudoToken == ModuleVersionIdStringToken)
                            {
                                ...
                                if (mvidStringHandle.IsNil)
                                {
                                    ...
                                    RoslynDebug.Assert(guidStringLength == default(Guid).ToString().Length);
        ...
        private void SerializeParameterInformation(ParameterTypeEncoder encoder, IParameterTypeInformation parameterTypeInformation)
        {
            ...
            {
                RoslynDebug.Assert(!parameterTypeInformation.IsByReference);
            ...
            {
                RoslynDebug.Assert(parameterTypeInformation.RefCustomModifiers.Length == 0 || parameterTypeInformation.IsByReference);
        ...
        private void SerializeCustomAttributeSignature(ICustomAttribute customAttribute, BlobBuilder builder)
        {
            ...
            RoslynDebug.Assert(parameters.Length == arguments.Length);
        ...
        private void SerializeMetadataExpression(LiteralEncoder encoder, IMetadataExpression expression, ITypeReference targetType)
        {
            if (expression is MetadataCreateArray a)
            {
                ...
                if (!(targetType is IArrayTypeReference targetArrayType))
                {
                    ...
                    RoslynDebug.Assert(this.module.IsPlatformType(targetType, PlatformType.SystemObject));
            ...
            else
            {
                ...
                if (c != null)
                {
                    ...
                    RoslynDebug.Assert(!module.IsPlatformType(c.Type, PlatformType.SystemType) || c.Value == null);
        ...
        private void SerializeMarshallingDescriptor(IMarshallingInformation marshallingInformation, BlobBuilder writer)
        {
            ...
            switch (marshallingInformation.UnmanagedType)
            {
                case UnmanagedType.ByValArray: // NATIVE_TYPE_FIXEDARRAY
                    RoslynDebug.Assert(marshallingInformation.NumberOfElements >= 0);
                ...
                case UnmanagedType.LPArray: // NATIVE_TYPE_ARRAY
                    RoslynDebug.Assert(marshallingInformation.ElementType >= 0);
                    writer.WriteCompressedInteger((int)marshallingInformation.ElementType);
        ...
        internal static string StrongName(IAssemblyReference assemblyReference)
        {
            ...
            {
                RoslynDebug.Assert(identity.ContentType == AssemblyContentType.Default);
            }
        ...
        private void SerializeReturnValueAndParameters(MethodSignatureEncoder encoder, ISignature signature, ImmutableArray<IParameterTypeInformation> varargParameters)
        {
            ...
            {
                RoslynDebug.Assert(!signature.ReturnValueIsByRef);
            ...
            {
                RoslynDebug.Assert(!signature.ReturnValueIsByRef);
                RoslynDebug.Assert(signature.RefCustomModifiers.IsEmpty);
                SerializeCustomModifiers(returnTypeEncoder.CustomModifiers(), signature.ReturnValueCustomModifiers);
            ...
            {
                RoslynDebug.Assert(signature.RefCustomModifiers.IsEmpty || signature.ReturnValueIsByRef);
        ...
        private void SerializeTypeReference(SignatureTypeEncoder encoder, ITypeReference typeReference)
        {
            while (true)
            {
                ...
                RoslynDebug.Assert(!module.IsPlatformType(typeReference, PlatformType.SystemTypedReference));
        ...
        private void SerializeCustomAttributeArrayType(CustomAttributeArrayTypeEncoder encoder, IArrayTypeReference arrayTypeReference)
        {
            ...
            RoslynDebug.Assert(arrayTypeReference.IsSZArray);

            var elementType = arrayTypeReference.GetElementType(Context);
            RoslynDebug.Assert(!(elementType is IModifiedTypeReference));
        ...
        private void SerializeCustomAttributeElementType(CustomAttributeElementTypeEncoder encoder, ITypeReference typeReference)
        {
            ...
            {
                RoslynDebug.Assert(typeReference.IsEnum);
        ...
        protected abstract class HeapOrReferenceIndexBase<T>
        {
            ...
            {
                RoslynDebug.Assert(!_writer._tableIndicesAreComplete);
#if DEBUG
                int i;
                RoslynDebug.Assert(!this.TryGetValue(item, out i));
#endif
        ...
        protected sealed class HeapOrReferenceIndex<T> : HeapOrReferenceIndexBase<T>
        {
            ...
            {
                RoslynDebug.Assert(index.Count == 0);
                _index = index;
            }
