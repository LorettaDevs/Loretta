...
namespace Loretta.Cci
{
    ...
    internal abstract class ExceptionHandlerRegion
    {
        ...
        {
            RoslynDebug.Assert(tryStartOffset < tryEndOffset);
            RoslynDebug.Assert(tryEndOffset <= handlerStartOffset);
            RoslynDebug.Assert(handlerStartOffset < handlerEndOffset);
            RoslynDebug.Assert(tryStartOffset >= 0);
            RoslynDebug.Assert(tryEndOffset >= 0);
            RoslynDebug.Assert(handlerStartOffset >= 0);
            RoslynDebug.Assert(handlerEndOffset >= 0);
    ...
    internal sealed class ExceptionHandlerRegionFilter : ExceptionHandlerRegion
    {
        ...
        public ExceptionHandlerRegionFilter(
            ...
            : base(tryStartOffset, tryEndOffset, handlerStartOffset, handlerEndOffset)
        {
            RoslynDebug.Assert(filterDecisionStartOffset >= 0);
