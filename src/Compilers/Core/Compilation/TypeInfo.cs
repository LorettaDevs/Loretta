...
namespace Loretta.CodeAnalysis
{
    public readonly struct TypeInfo : IEquatable<TypeInfo>
    {
        ...
        internal TypeInfo(ITypeSymbol? type, ITypeSymbol? convertedType, NullabilityInfo nullability, NullabilityInfo convertedNullability)
            : this()
        {
            RoslynDebug.Assert(type is null || type.NullableAnnotation == nullability.FlowState.ToAnnotation());
            RoslynDebug.Assert(convertedType is null || convertedType.NullableAnnotation == convertedNullability.FlowState.ToAnnotation());
            this.Type = type;
