    ...
    public abstract partial class Compilation
    {
            ...
            where T : CompilationReference
        {
            ...
            for (int i = 0; i < result.Length; i++)
            {
                ...
                {
                    RoslynDebug.Assert(reference is UnresolvedMetadataReference || reference is CompilationReference);
        ...
        private void AssertNoScriptTrees()
        {
            ...
            {
                RoslynDebug.Assert(tree.Options.Kind != SourceCodeKind.Script);
            }
        }
...
#pragma warning restore RS0026 // Do not add multiple public overloads with optional parameters
            ...
            checked
            {
                ...
                if (!noManifest)
                {
                    if (this.Options.OutputKind.IsApplication())
                    {
                        ...
                        {
                            manifestContents = typeof(Compilation).GetTypeInfo().Assembly.GetManifestResourceStream("Loretta.CodeAnalysis.Resources.default.win32manifest");
                        }
                    }
                    else
                    {
                        // Modules never have manifests, even if one is specified.
                        //RoslynDebug.Assert(!this.Options.OutputKind.IsNetModule() || manifestContents == null);
                    }
        ...
        internal bool CreateDebugDocuments(DebugDocumentsBuilder documentsBuilder, IEnumerable<EmbeddedText> embeddedTexts, DiagnosticBag diagnostics)
        {
            ...
            if (!embeddedTexts.IsEmpty())
            {
                ...
                {
                    RoslynDebug.Assert(!string.IsNullOrEmpty(text.FilePath));
        ...
        {
            RoslynDebug.Assert(IsSubmission);
            ...
            if (this.GetSubmissionSlotIndex() >= 0 && HasCodeToEmit())
            {
                ...
                RoslynDebug.Assert(this.CommonAnonymousTypeManager.AreTemplatesSealed);
            }
            else
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            RoslynDebug.Assert(!embedPdb || pdbStream == null);
            RoslynDebug.Assert(metadataPEStream == null || !options.IncludePrivateMembers); // you may not use a secondary stream and include private members together
            ...
            CancellationToken cancellationToken)
        {
            ...
            RoslynDebug.Assert(moduleBeingBuilt.DebugInformationFormat != DebugInformationFormat.Embedded || pdbStreamProvider == null);
            ...
            try
            {
                ...
                if (moduleBeingBuilt.DebugInformationFormat == DebugInformationFormat.Pdb && pdbStreamProvider != null)
                {
                    ...
                    RoslynDebug.Assert(!deterministic || moduleBeingBuilt.PdbChecksumAlgorithm.Name != null);
                ...
                try
                {
                        ...
                        cancellationToken))
                    {
                        if (nativePdbWriter != null)
                        {
                            ...
                            RoslynDebug.Assert(nativePdbStream != null || metadataDiagnostics.HasAnyErrors());
        ...
        private static Stream? ConditionalGetOrCreateStream(EmitStreamProvider metadataPEStreamProvider, DiagnosticBag metadataDiagnostics)
        {
            ...
            RoslynDebug.Assert(auxStream != null || metadataDiagnostics.HasAnyErrors());
            return auxStream;
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (emitSecondaryAssembly)
            {
                ...
                RoslynDebug.Assert(!includePrivateMembers);
        ...
        internal int CompareSyntaxTreeOrdering(SyntaxTree tree1, SyntaxTree tree2)
        {
            ...
            RoslynDebug.Assert(this.ContainsSyntaxTree(tree1));
            RoslynDebug.Assert(this.ContainsSyntaxTree(tree2));
        ...
        public static string? GetRequiredLanguageVersion(Diagnostic diagnostic)
        {
            ...
            if (diagnostic.Arguments != null)
            {
                foreach (var argument in diagnostic.Arguments)
                {
                    ...
                    {
                        RoslynDebug.Assert(!found); // only one required language version in a given diagnostic
                        found = true;
