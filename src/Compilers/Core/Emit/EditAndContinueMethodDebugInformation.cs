...
namespace Loretta.CodeAnalysis.Emit
{
    ...
    public readonly struct EditAndContinueMethodDebugInformation
    {
        ...
        {
            RoslynDebug.Assert(methodOrdinal >= -1);
        ...
        internal void SerializeLocalSlots(BlobBuilder writer)
        {
            ...
            foreach (LocalSlotDebugInfo localSlot in LocalSlots)
            {
                ...
                RoslynDebug.Assert(kind <= SynthesizedLocalKind.MaxValidValueForLocalVariableSerializedToDebugInformation);
        ...
        internal void SerializeLambdaMap(BlobBuilder writer)
        {
            ...
            foreach (LambdaDebugInfo info in Lambdas)
            {
                RoslynDebug.Assert(info.ClosureOrdinal >= LambdaDebugInfo.MinClosureOrdinal);
                RoslynDebug.Assert(info.LambdaId.Generation == 0);
