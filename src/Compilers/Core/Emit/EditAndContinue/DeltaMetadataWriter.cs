    ...
    internal sealed class DeltaMetadataWriter : MetadataWriter
    {
        ...
        public DeltaMetadataWriter(
            ...
            : base(metadata: MakeTablesBuilder(previousGeneration),
                   ...
                   cancellationToken: cancellationToken)
        {
            RoslynDebug.Assert(previousGeneration != null);
            RoslynDebug.Assert(encId != default(Guid));
            RoslynDebug.Assert(encId != previousGeneration.EncId);
            RoslynDebug.Assert(context.Module.DebugInformationFormat != DebugInformationFormat.Embedded);
        ...
        private static IReadOnlyDictionary<K, V> AddRange<K, V>(IReadOnlyDictionary<K, V> previous, IReadOnlyDictionary<K, V> current, bool replace = false, IEqualityComparer<K> comparer = null)
        {
            ...
            {
                RoslynDebug.Assert(replace || !previous.ContainsKey(pair.Key));
            ...
            where T : class, IDefinition
        {
            switch (_changes.GetChange(def))
            {
                ...
                case SymbolChange.Updated:
                    ...
                    return false;
                case SymbolChange.ContainsChanges:
                    RoslynDebug.Assert(def is INestedTypeDefinition);
            ...
            where T : class, ITypeDefinitionMember
        {
            foreach (var member in index.GetRows())
            {
                if (index.IsAddedNotChanged(member))
                {
                    ...
                    RoslynDebug.Assert(typeIndex > 0);
        ...
        protected override void PopulateEncMapTableRows(ImmutableArray<int> rowCounts)
        {
            ...
            RoslynDebug.Assert(tokens.Distinct().Count() == tokens.Count);
            ...
            if (_debugMetadataOpt != null)
            {
                ...
                // Should not be any duplicates.
                RoslynDebug.Assert(debugTokens.Distinct().Count() == debugTokens.Count);
            ...
            for (int i = 0; i < rowCounts.Length; i++)
            {
                ...
                RoslynDebug.Assert(rowCounts[i] == 0);
            }
#endif
        }
        ...
        private sealed class DefinitionIndex<T> : DefinitionIndexBase<T> where T : class, IDefinition
        {
...
#if DEBUG
                    T other;
                    RoslynDebug.Assert(!_map.TryGetValue(index, out other) || ((object)other == (object)item));
#endif
                    _map[index] = item;
                    return true;
                }
            ...
            {
                RoslynDebug.Assert(!this.IsFrozen);
        ...
        private sealed class DeltaReferenceIndexer : ReferenceIndexer
        {
            ...
            {
                RoslynDebug.Assert(this.ShouldVisit(eventDefinition));
                base.Visit(eventDefinition);
            }

            public override void Visit(IFieldDefinition fieldDefinition)
            {
                RoslynDebug.Assert(this.ShouldVisit(fieldDefinition));
            ...
            {
                RoslynDebug.Assert(this.ShouldVisit(method));
                base.Visit(method);
            }
