...
namespace Loretta.CodeAnalysis.Emit
{
    ...
    internal struct MethodImplKey : IEquatable<MethodImplKey>
    {
        ...
        {
            RoslynDebug.Assert(implementingMethod > 0);
    ...
    public sealed class EmitBaseline
    {
        ...
        internal sealed class MetadataSymbols
        {
            ...
            {
                RoslynDebug.Assert(anonymousTypes != null);
                RoslynDebug.Assert(metadataDecoder != null);
                RoslynDebug.Assert(assemblyReferenceIdentityMap != null);
            ...
            IReadOnlyDictionary<MethodImplKey, int> methodImpls)
        {
            ...
            RoslynDebug.Assert((ordinal == 0) == (encId == default));
            RoslynDebug.Assert((ordinal == 0) == (initialBaseline == null));
            RoslynDebug.Assert((ordinal == 0) == (anonymousTypeMap == null));
            RoslynDebug.Assert(encId != module.GetModuleVersionId());
            RoslynDebug.Assert(debugInformationProvider != null);
            RoslynDebug.Assert(localSignatureProvider != null);
            RoslynDebug.Assert(typeToEventMap != null);
            RoslynDebug.Assert(typeToPropertyMap != null);
            RoslynDebug.Assert(moduleVersionId != default);
            RoslynDebug.Assert(moduleVersionId == module.GetModuleVersionId());
            RoslynDebug.Assert(synthesizedMembers != null);

            RoslynDebug.Assert(tableEntriesAdded.Length == MetadataTokens.TableCount);
            ...
            RoslynDebug.Assert(tableEntriesAdded[(int)TableIndex.TypeDef] >= typesAdded.Count);
            RoslynDebug.Assert(tableEntriesAdded[(int)TableIndex.Event] >= eventsAdded.Count);
            RoslynDebug.Assert(tableEntriesAdded[(int)TableIndex.Field] >= fieldsAdded.Count);
            RoslynDebug.Assert(tableEntriesAdded[(int)TableIndex.MethodDef] >= methodsAdded.Count);
            RoslynDebug.Assert(tableEntriesAdded[(int)TableIndex.Property] >= propertiesAdded.Count);
            RoslynDebug.Assert(tableEntriesAdded[(int)TableIndex.EventMap] >= eventMapAdded.Count);
            RoslynDebug.Assert(tableEntriesAdded[(int)TableIndex.PropertyMap] >= propertyMapAdded.Count);
        ...
        internal EmitBaseline With(
            ...
            Func<MethodDefinitionHandle, StandaloneSignatureHandle> localSignatureProvider)
        {
            RoslynDebug.Assert(_anonymousTypeMap == null || anonymousTypeMap != null);
            RoslynDebug.Assert(_anonymousTypeMap == null || anonymousTypeMap.Count >= _anonymousTypeMap.Count);
        ...
        internal IReadOnlyDictionary<AnonymousTypeKey, AnonymousTypeValue> AnonymousTypeMap
        {
            get
            {
                if (Ordinal > 0)
                {
                    RoslynDebug.Assert(_anonymousTypeMap is object);
                    return _anonymousTypeMap;
                }

                RoslynDebug.Assert(LazyMetadataSymbols != null);
        ...
        private static Dictionary<int, int> CalculateTypePropertyMap(MetadataReader reader)
        {
            ...
            {
                RoslynDebug.Assert(!parentType.IsNil);
