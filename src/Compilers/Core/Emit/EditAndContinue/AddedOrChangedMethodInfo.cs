...
namespace Loretta.CodeAnalysis.Emit
{
    internal struct AddedOrChangedMethodInfo
    {
            ...
            ImmutableArray<Cci.ITypeReference> stateMachineAwaiterSlotsOpt)
        {
            ...
            RoslynDebug.Assert(methodId.Generation >= 0);

            // each state machine has to have awaiters:
            RoslynDebug.Assert(stateMachineAwaiterSlotsOpt.IsDefault == (stateMachineTypeNameOpt == null));

            // a state machine might not have hoisted variables:
            RoslynDebug.Assert(stateMachineHoistedLocalSlotsOpt.IsDefault || (stateMachineTypeNameOpt != null));
        ...
        {
            RoslynDebug.Assert(!info.IsDefault);
            if (info.IsUnused)
            {
                RoslynDebug.Assert(info.Signature != null);
                return info;
            }
