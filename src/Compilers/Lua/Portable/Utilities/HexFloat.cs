...
namespace Loretta.CodeAnalysis.Lua.Utilities
{
    internal static class HexFloat
    {
        ...
        public static string DoubleToHexString(double x)
        {
            ...
            var xn = unchecked((ulong) BitConverter.DoubleToInt64Bits(x));
            var sign = (int) (xn >> (maxBits - 1 + expBits));
            var e = (int) ((xn >> (maxBits - 1)) & maxBiasedExp); // the biased exponent
            ...
            if (e < maxBiasedExp)
            {
                ...
                {
                    var h = unchecked((int) (s >> ((maxFractNibbles - 1 - j) << 2))) & 0xf;
