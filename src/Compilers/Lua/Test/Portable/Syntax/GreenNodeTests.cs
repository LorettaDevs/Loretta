...
public partial class GreenNodeTests
{
    private static CodeAnalysis.Syntax.InternalSyntax.SeparatedSyntaxList<T> SeparatedSyntaxList<T>(params T[] args)
        where T : GreenNode
    {
        if (args.Length < 1) return new CodeAnalysis.Syntax.InternalSyntax.SeparatedSyntaxList<T>();
    ...
    private sealed class IdentityRewriter : InternalSyntax.LuaSyntaxRewriter
    {
