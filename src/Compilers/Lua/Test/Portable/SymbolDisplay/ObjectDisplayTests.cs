...
public sealed class ObjectDisplayTests
{
    ...
    public void ObjectDisplay_FormatLiteralString_ThrowsExceptionWhenValueIsNull() =>
        Assert.Throws<ArgumentNullException>(static () => ObjectDisplay.FormatLiteral(null!, ObjectDisplayOptions.None));
    ...
    public void ObjectDisplay_FormatLiteralString_OnlyAddsQuotesWhenAskedTo()
    {
        ...
        Assert.False(
            noQuotes.StartsWith("\"", StringComparison.Ordinal)
            || noQuotes.EndsWith("\"", StringComparison.Ordinal),
            "No quotes output has quotes.");
        Assert.True(
            withQuotes.StartsWith("\"", StringComparison.Ordinal)
            || withQuotes.EndsWith("\"", StringComparison.Ordinal),
            "With quotes output has no quotes.");
    }
