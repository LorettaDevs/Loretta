...
- Added new node `FunctionTypeParameterSyntax` that represents a parameter to a function type/signature by @GGG-KILLER in **TODO-119**.
...
- **[Breaking]** `FunctionTypeSyntax.Parameters` (and associated method parameters) have been switched from a `SeparatedSyntaxList<TypeSyntax>` into a `SeparatedSyntaxList<FunctionTypeParameterSyntax>` by @GGG-KILLER in **TODO-119**.
- **[Breaking]** `SyntaxFactory.FunctionType` now accepts a `SeparatedSyntaxList<FunctionTypeParameterSyntax>` instead of a `SeparatedSyntaxList<TypeSyntax>` by @GGG-KILLER in **TODO-119**. 
